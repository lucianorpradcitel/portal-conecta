...
import { IconeCaixa, IconeLupa } from './ui/Icones'
import {
  BotaoAtualizar,
  BotaoLimpar,
  CampoBusca,
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={aoVoltar}
            className="inline-flex items-center gap-1.5 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 transition-colors duration-150"
          >
            <span aria-hidden="true">←</span> Integrações
          </button>
          <div>
            ...
            <p className="text-sm text-slate-500">Pedidos do lojista em todos os status.</p>
          </div>
        </div>
...
function CartaoStatus({
  ...
  return (
    <button
      ...
      onClick={aoClicar}
      aria-pressed={ativo}
      className={`text-left bg-white rounded-lg border shadow-sm p-4 flex items-center gap-3 transition-all duration-150 hover:shadow ${
        ativo ? 'border-indigo-500 ring-2 ring-indigo-500/20' : 'border-slate-200 hover:border-slate-300'
      }`}
    >
      <div className={`h-10 w-10 shrink-0 rounded-lg flex items-center justify-center text-sm font-semibold ${cor}`}>
        {codigo ?? '∑'}
      </div>
      <div>
...
function Paginacao({
  ...
  const botao =
    'inline-flex h-8 min-w-[2rem] items-center justify-center rounded-md border border-slate-300 bg-white px-2 text-sm font-medium text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:text-slate-300 disabled:hover:bg-white'
  return (
